...
export default function AssetDetailPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (aErr || !a) { setErr(aErr?.message || "Asset not found"); return; }
      setAsset(a);
      setForm(a);
  ...
  const onSave = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      cost: form.cost !== undefined && form.cost !== null ? Number(form.cost) : null,
    ...
    // refresh local asset state
    setAsset((prev) => prev ? { ...prev, ...(payload as any) } : prev);
    alert("Asset updated");
  };
  ...
  return (
    <main className="p-8 bg-gray-50 min-h-screen">
      ...
      <form onSubmit={onSave} className="bg-white border rounded p-6 max-w-2xl space-y-4">
      ...
      {company && (
        <div className="mt-6 bg-white border rounded p-4 max-w-2xl">
          ...
          <div className="text-sm text-gray-700">
            Company: <span className="font-medium">{company.name}</span><br />
            Rate: {company.depreciation_rate ?? 0}% per year<br />
